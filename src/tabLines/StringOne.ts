import { PlayedNoteOneSilence } from 'src/notes/playedNote/PlayedNoteOneSilence';
...
export default class StringOne {
  ...
  private fromNotesToTabNotes(
    ...
    let previousNote: PlayedNoteOneInterface = notes[0];
    let currentNote: PlayedNoteOneInterface = notes[0];
    notes.forEach((note) => {
      currentNote = note;
